...
class GameSession:
    ...
    def process_input(self, text: str, ai: GigaAI) -> list[tuple[str, list[str] | None]]:
        """Обработка ввода"""
        reply = []
        ...
        if action_id is None:
            raw_response = ai.parse_action(text, self.current_location.name, self.current_scene.available_actions)
            action_id = self.__pr.preprocess(raw_response)[0]["action"]
            action = self.current_location.ai_actions[action_id]

            raw_response = ai.parse(text, action.pattern, self.current_location.name, ["Elsa", "Bard"])
            action_params = self.__pr.preprocess(raw_response)[0]
